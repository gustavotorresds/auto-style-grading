...
public class BreakoutExtension extends GraphicsProgram {
	...
	public  double VELOCITY_Y = 3.0;
	
	// The coordinates of where the end message will be placed on the screen 
	public static final int END_MESSAGE_X = 10;
	public static final int END_MESSAGE_Y =  (int)BRICK_Y_OFFSET - 20;
	...
	// message displayed when game is won
	public static final String WINNING_MESSAGE = "Congrats – you broke all the bricks!";
	
	// message displayed if all three lives are lost before winning
	public static final String LOSING_MESSAGE = "You lost before you broke out :(";
	...
	// labels on screen which keep track of lives left and bricks hit
	private GLabel livesLeft = new GLabel("Lives Left: " + lives, CANVAS_WIDTH - 120, BRICK_Y_OFFSET - 30);
	private GLabel bricksHit = new GLabel ("Bricks Hit: " + bricksRemoved, CANVAS_WIDTH - 120, BRICK_Y_OFFSET - 15);
	
	// color of bricks that will be printed in a given row
	private Color brickColor;
	
	// sounds ball makes whenever it bounces off of something
	...
	private void setVelocity() {
		// vy increases bye one for everyone set of 7 bricks removed
		vy = VELOCITY_Y;
	 ...
	 * Method: updateBricks
	 * -------------------
	 * This method creates a label on the window which tracks how many bricks 
	 * the player has hit. 
	 */
	private void updateBricks() {
		bricksHit = new GLabel ("Bricks Hit: " + bricksRemoved, CANVAS_WIDTH - 120, 30);
		bricksHit.setColor(Color.blue);
		bricksHit.setFont("Monospaced");
		add(bricksHit);
	}
	 ...
	 * Method: updateLives
	 * -------------------
	 * This method creates a label on the window which tracks how many lives 
	 * the player has left. 
	 */
	private void updateLives() {
		livesLeft = new GLabel ("Lives Left: " + lives, CANVAS_WIDTH - 120, 15);
		livesLeft.setColor(Color.magenta);
		livesLeft.setFont("Monospaced");
		add(livesLeft);
	}
	 ...
	 * -------------------
	 * This method displays appropriate message on window after game is over.
	 */
	private void addEndMessage(){
		GLabel closingWords;
		if (hasWon()) {
			closingWords = new GLabel (WINNING_MESSAGE, END_MESSAGE_X, END_MESSAGE_Y);
			closingWords.setColor(Color.green);
		} else {
			closingWords = new GLabel (LOSING_MESSAGE, END_MESSAGE_X, END_MESSAGE_Y);
			closingWords.setColor(Color.red);
		}
		closingWords.setFont("Monospaced-BOLD-18");
		add(closingWords);
	}
}
