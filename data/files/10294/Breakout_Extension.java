...
public class Breakout_Extension extends GraphicsProgram {
	...
	public void run() {
		...
		// On my computer I need to start the program with the sound in order for it to run properly.
		bounceClip.play();
		setUpGame();
		// Runs the game only if there are still bricks. Gives 3 balls (3 chances to play)
		
			for (numRounds = 0; numRounds < NTURNS; numRounds++) {
				if (totalBrickNum >0) {
					playGame();
					if (numRounds == NTURNS - 1) {
						gameLostMessage();
					}
				}
			}
	...
	private void playGame() {
		ball = createBall();
		addBall();
		addBallsRemainingMessage();
		animationLoop();
		remove(ball);
		//fix sideways ball on paddle, velocity change, add counter and win/loss message.
	}
	...
	private void gameLostMessage() {
		GLabel gameLost = new GLabel("Out of balls: You lose!");
		gameLost.setFont("Courier-24");
		gameLost.setColor(Color.RED);
		gameLost.setLocation((getWidth() - gameLost.getWidth())/2, (getHeight() + gameLost.getAscent())/2);
		add(gameLost);
	}
	
	private void gameWinningMessage() {
		GLabel gameWon = new GLabel("You Win!");
		gameWon.setFont("Courier-24");
		gameWon.setColor(Color.GREEN);
		gameWon.setLocation((getWidth() - gameWon.getWidth())/2, (getHeight() + gameWon.getAscent())/2);
		add(gameWon);
	}
	
	private void addBallsRemainingMessage() {
		GLabel startMessage = new GLabel("Click to play!");
		startMessage.setFont("Courier-24");
		startMessage.setColor(Color.BLUE);
		startMessage.setLocation((getWidth() - startMessage.getWidth())/2, 
								 (getHeight() + startMessage.getAscent())/2 - 4 * BALL_RADIUS);
		//gives a message each turn with the number of balls remaining if it is past turn 1.
		GLabel ballsRemaining = new GLabel(NTURNS - numRounds + " ball(s) remaining!");
		ballsRemaining.setFont("Courier-24");
		ballsRemaining.setColor(Color.BLUE);
		ballsRemaining.setLocation((getWidth() - ballsRemaining.getWidth())/2, 
								 (getHeight() + ballsRemaining.getAscent())/2 - 4 * BALL_RADIUS);
	...
	private void displayScore() {
		GLabel pointDisplay = new GLabel("Points: "  + addBrickPointValues());
		pointDisplay.setLocation((getWidth() - pointDisplay.getWidth())/2, getHeight() - PADDLE_Y_OFFSET/4);
		remove(pointDisplay);
		add(pointDisplay);
	}
	
	/* This method should find the color of the object the ball collides with and a corresponding 
	 * value. The values are to be added up and displayed with the GLabel pointDisplay.
	 * 
	 */
	private int addBrickPointValues() {
		GObject collider = getCollidingObject();
		if (collider != null) {
