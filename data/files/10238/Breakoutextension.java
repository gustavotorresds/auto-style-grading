 ...
 * 
 * This extension tells the user to click to start, makes a sound when the paddle hits the ball, 
 * keeps score, and also adds to the win and loss message at the end. 
 */
...
public class Breakoutextension extends GraphicsProgram {
	...
	//instance variable that takes in input about collisions with the ball
	...
	//instance variable for the Glabel that tells the user to click
	private GLabel clickToStart;
	
	//instance variable for keeping track of the number of times the 
	//ball hits the paddle.
	private int paddlehits;
	
	//instance variable that tracks the points the user gets from hitting the ball
	private int score;
	
	//instance variable for the score displayed on the screen. 
	...
	public void run() {
		...
		add(scorelabel, (getWidth()- scorelabel.getWidth())/2, getHeight()- scorelabel.getAscent());
		...
		if (numberofbricks == 0) {
			...
			GImage youwin = new GImage ("chris.png");
			add(youwin, (getWidth()-youwin.getWidth())/2, BRICK_Y_OFFSET - youWin.getAscent());
		}
		...
		if (turnsused == NTURNS && numberofbricks != 0) {
			 ...
			 GImage youlose = new GImage ("chris-teaching.png");
			 youlose.setSize(CANVAS_WIDTH/3, CANVAS_WIDTH/3);
			 add(youlose, (getWidth()-youlose.getWidth())/2, getHeight()/2);
		}
	}
	...
	private void setUpGame() {
		...
		add (clickToStart,(getWidth()-clickToStart.getWidth())/2, (getHeight()-clickToStart.getAscent())/2 - 10);
	...
	private void keepScore() {
		//adds to the score depending on which brick you hit. the higher the brick 
