/*
 * File: Breakout Extension
 * -------------------
 * Name: Ana Martins
 * Section Leader:Luciano Gonzales
 * 
 * This program runs the game of Breakout with labels
 */
...
public class BreakoutExtension extends GraphicsProgram {
	 ...
	 * -----------
	 * Extension
	 * Adds labels depending on end result
	 */
	
	private void addLabels() {
		if (gameOver()) {
			GLabel loser = new GLabel("just take the L...");
			loser.setFont("Courier-30");
			loser.setColor(Color.RED);
			add(loser, getCenterX() - loser.getWidth()/2 , getCenterY() - loser.getHeight()/2);
		}
		if (userWins()) {
			GLabel winner = new GLabel("what a champ");
			winner.setFont("Courier-30");
			winner.setColor(Color.GREEN);
			add(winner, getCenterX() - winner.getWidth()/2 , getCenterY() - winner.getHeight()/2);
		}
	}
