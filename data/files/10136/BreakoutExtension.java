...
public class BreakoutExtension extends GraphicsProgram {
		...
		while(true) {
			...
			if (ball.getY()+BALL_RADIUS*2 >= getHeight()) {
				...
				if (turnsLeft == 0) { 
					remove(ball);
					YouLost();
					AudioClip loseClip = MediaTools.loadAudioClip("lose.mp3");
					loseClip.play();
					break;
				} else {
					remove(ball);
			...
			if (BrickCount == 0) {
				remove(ball);
				YouWon();
				AudioClip winClip = MediaTools.loadAudioClip("win.mp3");
				winClip.play();
				break; 
			}
		}
	...
	private void YouLost() {
		GImage image = new GImage("youlose.jpg"); 
	...
	private void YouWon() {
		GImage image = new GImage("youwin.jpg"); 
		image.setLocation(0, 0);
		image.setSize(getWidth(), getHeight());
		add(image); 
	}
