 ...
 * 
 * Implements the game of Breakout with goodies.
 */
...
public class BreakoutExtension extends GraphicsProgram {
	...
	public static final double VELOCITY_Y = 6.0; // warning: if larger than brick height, collision skipping errors may occur
	...
	/* 
	 * Prompts the player to click, waits for click to begin game
	 */
	
	private void startUpMessage() {
		GLabel label = new GLabel("Click to play");
		GLabel click = new GLabel("Click to play", getWidth()/2 - label.getWidth()/2, getHeight()/2 - label.getHeight()/2);
		add(click);
		waitForClick();
		remove(click);
	}
	...
	/*
	 * Displays heart graphics for number of lives
	 */

	private void displayLives(int lives) {
		for (int i = 0; i < lives; i++) {
			GImage heart = new GImage("heart.png", 10+(25*i), getHeight() - 30);
			heart.scale(.25);
			add(heart);
		}		
	}
	...
	/*
	 * Removes one heart graphic after a turn ends
	 */
	
	private void removeLives(int lives) {
		remove(getElementAt(25 * (lives+1) , getHeight() - 20));
	}
