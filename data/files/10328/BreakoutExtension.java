...
public class BreakoutExtension extends GraphicsProgram {
	...
	// Instance variables for objects in the program (specifically the ball,
	// paddle, and bricks)
	private GOval ball;
	...
	// Instance Variable for Audio 
		AudioClip bounceClip = MediaTools.loadAudioClip("bounce.au");

	// Keeps track of things including how many bricks are left, how many turns
	// were used,
	// the starting x coordinate of the bricks, and what the object being hit by
	// the ball is
	...
	/*
	 * This is the code for the computer game brick breaker. In this game, the
	 * user moves a paddle at the bottom of the screen around by moving their
	 * mouse. They are trying to keep a bouncing ball from falling to the ground
	 * and have the ball hit (and thus remove) a chunk of bricks from the top of
	 * the screen. Every time the ball falls to the ground the user looses a
	 * life; in this version of the game the user gets 3 lives. The user wins by
	 * removing all the bricks from the screen and looses if the ball falls to
	 * the ground 3 times before this task is completed.
	 */
	...
	private void animateBall() {
		vx = rgen.nextDouble(VELOCITY_X_MIN, VELOCITY_X_MAX); // Generates a
																// random
																// starting x
																// velocity
																// between the
																...
																// velocities
		vy = VELOCITY_Y;
		if (rgen.nextBoolean(0.5)) { // This makes the x velocity negative half
										// the time so when the
			vx = -vx; // ball is initially fired it doesn't always go to the
						// right
		}
		int currentTurns = turnsUsed;
		while (bricksLeft > 0 && currentTurns == turnsUsed) { // The ball should
																// move when the
																// bricks aren't
																// cleared and
																// the max
	...
	// Checks for collisions between the ball and bricks. It checks all 4
	// corners, one at a time, only
	// moving on to check the next corner if the first one didn't hit a brick.
	// When a brick is hit
	...
	// General method for checking for brick collisions. If one corner of the
	// ball hits a brick and
	// this method is executed, the brick is removed, the Y velocity of the ball
	// switches directions, the x velocity increase by 1.2 times, 
	...
	// Checks if the ball has hit the paddle. If it does hit the paddle, the y
	// velocity
	// switches directions.
	...
	// Checks if the ball hits one of the four walls. Something different
	// happens to the ball
	...
	private void checkForCollisionsWithWall() {
		...
		if (ball.getX() - BALL_RADIUS * 2 + diameter <= 0) { // If the ball hits
																// the left wall
			vx = -vx; // The X velocity switches
			bounceClip.play();
		}
		if (ball.getY() - BALL_RADIUS * 2 <= 0) { // If the ball hits the top
													// wall
			vy = -vy; // The Y velocity switches
			bounceClip.play();
		}
		if (ball.getY() >= getHeight() - BALL_RADIUS * 2) { // If the ball hits
															// the bottom wall
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (newXCoord <= getWidth() - PADDLE_WIDTH) { // Keeps the paddle within
														// the bounds of the
														// canvas
			paddle.setLocation(newXCoord, getHeight() - PADDLE_Y_OFFSET); // Resets
																			// paddle's
																			// location
																			// to
																			// the
	...
	// Builds the ball and adds it to the middle of the screen for the start of
	// the game
	...
	// Builds the chunk of bricks at the top of the screen
	private void buildBricks() {
		firstBrickXCoord = getWidth() / 2 - (NBRICK_COLUMNS * (BRICK_WIDTH + BRICK_SEP) / 2);
		for (int row = 0; row < NBRICK_ROWS; row++) {
			for (int brick = 0; brick < NBRICK_COLUMNS; brick++) {
				buildOneBrick(Color.WHITE, (firstBrickXCoord + brick * (BRICK_WIDTH + BRICK_SEP)),
						row * (BRICK_HEIGHT + BRICK_SEP) + BRICK_Y_OFFSET);
			}
		}
	}

	// Basic method for building one brick. The rectangles made from this method
	// will all be the
	// same size and color, but will have different x and y coordinates
