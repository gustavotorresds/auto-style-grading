...
public class BreakoutExtension extends GraphicsProgram {
	...
	java.applet.AudioClip loseClip = MediaTools.loadAudioClip("lose-sound-effect.au");
	
	java.applet.AudioClip themeClip = MediaTools.loadAudioClip("theme.au");
	...
	private void openingMenu() {
		GLabel message1 = new GLabel("Welcome to Breakout! Destroy all the bricks to win.");
		message1.setColor(Color.GREEN);
		message1.setLocation(getWidth()/2 - message1.getWidth()/2, getHeight()/2 - message1.getAscent()/2);
		add(message1);
		waitForClick();
		remove(message1);
		
		GLabel message2 = new GLabel("Watch out! Ball increases speed with every exploded brick.");
		message2.setColor(Color.RED);
		message2.setLocation(getWidth()/2 - message2.getWidth()/2, getHeight()/2 - message2.getAscent()/2);
		add(message2);
		waitForClick();
		remove(message2);
	}
	...
	private void clickToStart() {
		...
		start.setLocation(getWidth()/2 - start.getWidth()/2, getHeight()/3);
		add(start);
		waitForClick();
		remove(start);
	}
	...
	private void collideWithOtherObjects() {
		...
		if (collider != null) {
			if (collider == paddle) {
				...
				bounceClip.play();
				brickCounter--;
				vy = -1.007 * vy; //increases ball speed
			}
		}
	}
