 ...
 * 
 * Implements the game of Breakout for a specified number of turns, with the following extensions:
 * Game start and end, 'win' and 'fail' labels
 * Turn Counter
 * Sound for brick removal
 * Variable speed, getting faster as number of bricks reduces 
 * "Remaining Bricks" Counter
 * Clicks for start and next turn
 */
...
public class Breakout_Extension extends GraphicsProgram {
	...
	private int bricksLeft = 0; // instance variable to count number of bricks removed
	private int winCount = 0; // instance variable to count number of successful turns
	private GLabel gameLabel = null; // instance variable for game message label, e.g. 'click to start'
	private GLabel turnLabel = null; // instance variable for turn message label, e.g. turn successful or fail
	private GLabel brickCountLabel = null; // instance variable for label showing number of bricks remaining
	...
	public void run() {
		...
		// Initialize and play the game, for the specified number of turns.		
	 ...
	 * Method: Setup Game
	 * ------------------
	 * Initializes the breakout game, resetting the bricks, paddle, ball, velocity, and labels. Takes the turn number
	 * as input. 
	 */		
	private void setupGame(int turnCount) {
		...
		gameLabel = makeLabel();	// Initializes game message label
		gameLabel.setLabel("Welcome to Breakout! Click to Start: Turn " + turnCount + "/" + NTURNS);
		add(gameLabel, (getWidth() - gameLabel.getWidth()) / 2, (getHeight() - 2 * gameLabel.getHeight()) / 2);
		turnLabel = makeLabel();	// Initializes turn message label
	 ...
	 * Method: Play Game
	 * ------------------
	 * Plays the breakout game once by starting and running the animation loop, displays turn start and end messages. 
	 * Takes the turn number as input
	 */		
	private void playGame(int turnCount) {
		waitForClick(); // Click to start
		remove(gameLabel);	// Remove game message
		brickCountLabel = makeLabel();	// Initializes brick count label
		brickCountLabel.setLabel("Bricks Remaining: " + bricksLeft);
		add(brickCountLabel, (getWidth() - brickCountLabel.getWidth()) / 2, gameLabel.getHeight()); // Add brick count label
		boolean continueGame = true;// Initialize the check for end of game, used by the while loop
		...
		add(turnLabel, (getWidth() - turnLabel.getWidth()) / 2, (getHeight() - turnLabel.getHeight()) / 2);
		waitForClick();
		if (turnCount == NTURNS) { 
			remove(turnLabel);
			gameLabel.setLabel("Game Over! Number of wins: " + winCount + "/" + NTURNS);
			add(gameLabel, (getWidth() - gameLabel.getWidth()) / 2, (getHeight() - gameLabel.getHeight()) / 2);
		}
	}
	 ...
	 * Method: Make Label
	 * -------------------
	 * Creates a label for in-game messages.
	 */
	...
	private boolean checkForGameEnd() {
		...
		if (!continueGame) {
			turnLabel.setLabel("Fail! Click to Continue");
		}
		if (bricksLeft == 0) { // Checks for no bricks left
			continueGame = false;
			turnLabel.setLabel("Success! All bricks removed. Click to Continue");
			winCount++;  // Adds one to win count
		}
		return continueGame;
	}
	...
	private void checkForCollision() {
		...
		if (collider != null && collider != brickCountLabel) {
			bounceClip.play();
			...
			if (collider != paddle) {
				...
				remove(brickCountLabel);
				brickCountLabel.setLabel("Bricks Remaining: " + bricksLeft);
				add(brickCountLabel, (getWidth() - brickCountLabel.getWidth()) / 2, gameLabel.getHeight());
				if (bricksLeft > 10) {  // Speed increases with each brick removed
					vx = vx * 1.01; 	// Slower rate if more than 10 bricks left
				} else {
					vx = vx * 1.03;	// Faster for the last 10
				}
			}
		}
