/*
 * File: Breakout_Advanced.java
 * -------------------
 * Name: JiaJia Jin
 * Section Leader: N/A
 * 
 * This file will eventually implement the game of Breakout Advanced Version.
 */
...
public class Breakout_Advanced extends GraphicsProgram {
	...
	int score = 0;
	int numTurns = NTURNS; 
	int scenario = 0; // 1 = won; 2 = try again; 3 = the end
	
	// Load audio clip for bouncing sound
	...
	public void run() {
		...
		while(numTurns != 0) {
			...
			while (brickCount != 0) {
				...
				// Wrap up the turn, display "try again" message, take away
				// a life and reset brick count to total
	...
	/** Add a score board to the top left corner of screen
	 */
	private GLabel drawScoreboard() {
		scoreboard = new GLabel("Score: " + score);
		score = NBRICK_COLUMNS * NBRICK_ROWS - brickCount;
		scoreboard.setColor(Color.BLACK);
		scoreboard.setFont("Courier-12");
		add(scoreboard, 10, 10);
		return scoreboard;
	}
	...
	/** Update the score board when bricks are being removed
	 */
	...
	/** Check if ball is touching paddle on the edge when the collision happens
	 *  If so, need to reverse the ball's x velocity to make it more interesting
	 */
	private boolean ballIsOnEdge() {
		double ballPaddleDiffOnLeft = paddle.getX() - ball.getX();
		double ballPaddleDiffOnRight = paddle.getX() + PADDLE_WIDTH - ball.getX();
		return ballPaddleDiffOnLeft <= 2 * BALL_RADIUS && ballPaddleDiffOnLeft >= 0 
				|| (ballPaddleDiffOnRight <= 2 * BALL_RADIUS && ballPaddleDiffOnRight >=0);
	}
	
	/** Draw a message for user to indicate victory, defeat or another attempt 
	 *  Set location to center of screen, just below bricks
	 */
	private GLabel drawMessage(int scenario) {
		if (scenario == 1) {
			message = new GLabel("You won!");
		} else if (scenario == 2) {
			message = new GLabel("Let's try again!");
		} else {
			message = new GLabel("The end. Thanks for playing!");
		}
		message.setFont("Courier-12");
		add(message, getWidth() / 2 - message.getWidth() / 2, getHeight() / 3);
		return message;
	}
	
	/** When mouse clicks, remove all messages so that 
	 *  a new play can be played on a clean slate
	 */
