...
public class Breakout extends GraphicsProgram {
	...
	public void run() {
		...
		// Waits until user clicks to begin game.
		waitForClick();
		...
		// This adds a label to indicate the user lost the game and that it is the end of the game.  
		if (numberOfBricks != 0) {
			label = new GLabel ("Game Over! Better Luck Next Time!");
			label.setFont ("Courier-18");
			add(label, getWidth()/2 - (label.getWidth()/2), getHeight()/2 - (label.getAscent()/2)); 
			remove(ball);
		}
	...
	private void moveBall() {
		// This randomizes the velocity of X to be between the two constants.
		...
		while (true) {
			...
			// This initiates the aspect of the game 
			// to check when the ball hits collisions.
			checkForCollisions();
