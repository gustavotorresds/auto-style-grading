/*
 * File: Breakout with extensions.java
 * -------------------
 * Name: Yu Jin Lee
...
public class WithExtensions extends GraphicsProgram {
	...
	public static final double MESSAGE_Y_OFFSET = PADDLE_Y_OFFSET + 50;
	
	//display counter
	GLabel counter = new GLabel ("");
	public static final double COUNTER_X = CANVAS_WIDTH/2.0;
	public static final double COUNTER_Y_OFFSET = PADDLE_Y_OFFSET - 30;
	...
	public void run() {
		...
		waitForClick();
		remove (message);
		counter.setLabel("Remaining bricks: "+ NBRICK);
		add(counter, getHeight()-COUNTER_Y_OFFSET, COUNTER_X);
		...
		while ( NBRICK != 0 && top <= getHeight() - size) {
			...
			if (object != null) {
				...
				if (object != paddle) {
					//make bounce sound - how to reduce delay?
					bounceClip.play();
					remove (object);
					//counter
					NBRICK = NBRICK - 1;
		...
		if (NBRICK == 0) {
			putMessage("BREAKOUT WAS SUCCESSFUL!!!! CONGRATULATIONS!!!");
		}
		else if (top >= getHeight() - size) {
			putMessage("You lost. Please try again.");
		}

	}

	private void putMessage(String string) {
		message.setLabel(string);
		double labelX = getWidth()/2.0 - message.getWidth()/2.0;
		double labelY = getHeight()-MESSAGE_Y_OFFSET;
		add (message, labelX, labelY);
	}
