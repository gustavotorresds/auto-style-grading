...
public class BreakoutExtension extends GraphicsProgram {
	...
	public static final double VELOCITY_Y = 7; 
	...
	/*
	 * This program creates the game of Breakout by using different methods and instance variables.
	 * I have created several extensions.  I have created a scoreboard, I have added sound, I have changed the background color depending if a player wins or loses, I have added labels to a win or a loss, and I have added a wait for click.   
	 */
	...
	//this method simply creates the bricks, paddle, label for scoring
	private void canvasSetup() {
		...
		double labelx= getWidth()/2 - score.getWidth()/2;
		double labely= getHeight();
		add(score, labelx, labely);
	}
	...
	private void animationLoop() {
		while (true) {
			if (ball.getY() > getHeight()){  //ball goes to bottom
				remove(ball);
				ballcount ++;
				if (ballcount == NTURNS) {
					loseCondition();
			...
			if (collided == paddle) {
				...
				vy = -vy;
				remove(collided);
				removeBricksExtension();
				score.setText("score: "+ brickcount); //resets the score when a brick is hit
	...
	//this method creates the sound for when the brick is hit
	private void removeBricksExtension() {
