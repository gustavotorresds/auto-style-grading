...
public class Breakout extends GraphicsProgram {
	...
	// variables for paddle, ball, velocity, bricksleft, randomgen
	...
	public void run() {
		...
		for (int i = 0; i< NTURNS ; i++) {
			...
			createBall();
			playGame();
			// breaks out of method if game is won
			if(bricksleft == 0) {
				break;
			}
		}
...
// method for playing game
	private void playGame() {
		...
		while (true) {
			...
			// if ball doesn't hit anything it doesn't
			// enter loop, continues moving
			if (collider != null ){
				// if ball hits paddle velocity changes 
				if (collider == paddle) {
					vy = -1*(Math.abs(vy)); 
