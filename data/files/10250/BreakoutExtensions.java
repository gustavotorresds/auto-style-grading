 ...
 * 
 * This file creates and implements the game Breakout with a scoreboard and user-friendly messages.
 */
...
public class BreakoutExtensions extends GraphicsProgram {
	...
	//Instance variable for scoreboard
	GLabel scoreboard = null;
	...
	//Instance variable for user score count
	 ...
	 * loses for good, meaning they have no lives left. Score is tallied at the bottom, with 1 point for
	 * blue bricks, 2 points for green, 5 points for yellow, 10 points for orange, and 20 points for red.
	 */
	public void run() {
	...
	/** Creates a label at the bottom of the screen with the user's initial score and number of lives left */
	private void printInitialScore() {
		scoreboard = new GLabel ("Your Score: " + scoreCounter + "     Lives Left: " + livesLeft);
		double scoreX = getWidth()/2 - scoreboard.getWidth()/2;
		double scoreY = getHeight()/1.01;
		scoreboard.setFont("Serif-12");
		add (scoreboard, scoreX, scoreY);
	}

	/** Updates the scoreboard to display the user's score based on which bricks it has removed and number of lives left */
	private void updateInitialScore() {
		remove(scoreboard);
		printInitialScore();
	}

	/** Displays starting message and prompts user to click when ready. Upon click, message disappears */
	private void showStartMsg() {
		GLabel startMsg = new GLabel ("Ready to play? Just click!"); 
		startMsg.setFont("Serif-18");
		double labelX = getWidth()/2 - startMsg.getWidth()/2;
		double labelY = getHeight()/1.5;
		add (startMsg, labelX, labelY); 
		waitForClick();
		remove(startMsg);
	}

	/** If player removes all bricks, displays the "winning" message */
	...
	/** If ball's y-coordinate goes below the height of the screen, displays the "losing" message */
	private boolean checkIfLost() {
		if(livesLeft != 0) {
			showMessage("Oops! You have " + livesLeft + " lives left. Try again!");
			return false;
		} else {
	 ...
	 * if the collided element is not a paddle nor the scoreboard. bounceOffObject also keeps track of 
	 * number of bricks left (brickCounter) and the user's score based on brick color (scoreCounter)
	 * @param collidedElement
	 */
	private void bounceOffObject(GObject collidedElement) {
		if (collidedElement == paddle && collidedElement != scoreboard) {
			bounceClip.play();
			vy = Math.abs(vy)*-1;
		} else if (collidedElement != null && collidedElement != scoreboard) {
			vy = -vy;	
			bounceClip.play();
			remove (collidedElement);
			brickCounter--;
			if(collidedElement.getColor() == Color.RED) {
				scoreCounter = scoreCounter + 20;
			} else if(collidedElement.getColor() == Color.ORANGE) {
				scoreCounter = scoreCounter + 10;
			} else if(collidedElement.getColor() == Color.YELLOW) {
				scoreCounter = scoreCounter + 5;
			} else if(collidedElement.getColor() == Color.GREEN) {
				scoreCounter = scoreCounter + 2;
			} else if(collidedElement.getColor() == Color.CYAN) {
