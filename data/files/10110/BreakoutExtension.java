 ...
 * 
 * The BreakoutExtension class extends the GraphicsProgram class to play the classic arcade game of Breakout. It
...
public class BreakoutExtension extends GraphicsProgram {
	 ...
	 * Method: Set Up
	 * -----------------------
	 * This method sets up the initial conditions for the start of the game. The bricks and paddle are created 
	...
	private void setUp() {
		...
		drawLabel("Please click to begin");
		waitForClick();
		
		// removes initial get started label at the center of screen
		remove(getElementAt(cx, getHeight()/2));
	}

	/**
	 * Method: Play
	 * -----------------------
	 * This method launches and plays the game. It counts how many bricks are remaining and ends the game if
	 * there are no bricks left or if the ball reaches the bottom wall of the screen.
	 */
	...
	private int play(int bricksRemaining) {
		...
		//Loads sounds to be used in the game
		...
		//Manages gameplay while there are still bricks on the screen
		while(bricksRemaining > 0) {	
			...
			/* 
			 * If ball collides with an object that is not the paddle, the object is removed and 
			...
			if (collider != null && collider != paddle) {
				...
				// increases x velocity by 30% every five bricks up to a cap.
				if(bricksRemaining % 5 == 0) {
					if(Math.abs(vx) <= 8) {
						vx = vx * 1.3;
					}
				}
			...
			/*
			 * If the ball collides with the paddle, its x direction adjusts based on check paddle function and
			 * its y direction is reversed
			 */
			if (collider == paddle) {
				bounceClip.play();
				vx = checkPaddle(ball, vx);
				vy = -Math.abs(vy);
			}
			...
			//If the ball reaches the bottom wall, the turn ends
	 ...
	 * Method: End Game
	 * ------------------
	 * When the game ends, this method uses the number of bricks remaining to determine if
	 * a player has won or lost. It then calls drawLabel to add the appropriate message to the screen.
	 */
	 ...
	 * ------------------
	 * Controls the color of the bricks depending on which row is being created. Decimals are used rather than absolute
	 * numbers in the event that the number of brick rows is changed in the constants section. 
	 */
	 ...
	 * Method: Check Paddle
	 * --------------------
	 * Checks where the ball has hit the paddle and determines its x velocity. If it hits the left 20%,
	 * the ball heads in the left direction, if it hits in the right 20%, the ball heads in the right direction.
	 * Otherwise, its velocity stays the same.
	 */
	
	private double checkPaddle(GOval ball, double vx) {
		if (ball.getX() <= paddle.getX() + 0.2 * PADDLE_WIDTH) {
			vx = -Math.abs(vx);
		} else if (ball.getX() >= paddle.getX() + 0.8 * PADDLE_WIDTH) {
			vx = Math.abs(vx);
		}
		return vx;
	}
