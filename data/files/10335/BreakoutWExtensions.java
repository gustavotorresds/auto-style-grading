...
public class BreakoutwExtensions extends GraphicsProgram {
	...
	/*Method userFeedback: this method gives feedback to the user based on the
	 * performance. It allows the user to know how many turns are left and when the 
	 * game is over.
	 * Precondition: playGame.
	 * Postcondition: label based on playGame.
	 */
	public void userFeedBack() {
		if (!checkWall()) {
			GLabel label = new GLabel("You have " + (turntrack - 1) + " turn(s) left");
			add (label, getWidth()/2, getHeight()/2 + (BALL_RADIUS * 4));
		} if (totalbricks == 0) {
			GLabel label1 = new GLabel("You win. Game over");
			add (label1, getWidth()/2, getHeight()/2 + (BALL_RADIUS * 4));
		}
	}
}