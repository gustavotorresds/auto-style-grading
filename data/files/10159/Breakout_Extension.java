...
public class Breakout_Extension extends GraphicsProgram {
	 ...
	 * the brick is removed. If the ball collides into the paddle or a brick, a noise is 
	 * played. After each brick is removed, how many bricks remaining is displayed on the canvas. 
	 */
	private void collision() {
		...
		if (collider != null && collider !=paddle) {
			...
			count = new GLabel ("Bricks Left: " + numOfBricks);
			add (count, getWidth()/2 - count.getWidth()/2, getHeight() - PADDLE_Y_OFFSET/4.0);
			vy = -vy;
			bounceClip.play();
		}
	}
	...
	/* Adds colored bricks and paddle to the screen. Tells user to
	 * click to start the game.
	 */
	private void setUpGame() {
		start = new GLabel ("Click to begin game.");
		double cx = getWidth()/2;
		double cy = getHeight()/2;
		add (start, cx - start.getWidth()/2.0, cy - start.getAscent() - 2*BALL_RADIUS);
		addBricks();
		paddle = makePaddle();
		addPaddleToCenter();
	} 
