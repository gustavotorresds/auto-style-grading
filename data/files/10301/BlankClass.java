 ...
 * ---------------------
 * This class is an extension that differs from breakout
 * by having a setup sequence, scorekeeper, lives left tracker,
 * and sound.
 */
import acm.graphics.*;
import acm.program.*;
...
public class BlankClass extends GraphicsProgram {
	...
	public void run() {
		...
		for(int round =0; round <NTURNS; round++) {
			GLabel lives = new GLabel ("Lives Left:" + (3- round), getWidth()/2, getHeight()/2);
			lives.move(-lives.getWidth()/2,  -lives.getHeight());
	...
	//sets the screen for before the game starts
	private void buildUp() {
		...
		waitForClick();
		remove(welcome);
		GLabel ready = new GLabel ("Are You Ready?", getWidth()/2, getHeight()/2);
		ready.move(-ready.getWidth()/2,  -ready.getHeight());
	...
	private void moveBall() {
		...
		//problem if the ball is hit by the side of the paddle
		if(collider == paddle) {
			...
			if(ball.getY() >= paddle.getY() - BALL_RADIUS*2 && ball.getY() < getHeight() - PADDLE_Y_OFFSET-PADDLE_HEIGHT-BALL_RADIUS*2+4) {
				//makes sure the ball is hitting the top of paddle
			...
			remove(collider);
			brickCounter--;
			score.setFont("Times New Roman");
			score.setColor(Color.BLACK);
			score.setLabel("Your Score: " + (100 - brickCounter)*10);
			add(score, getWidth()/2-40, getHeight());
			vy = -vy;
			AudioClip bounceClip = MediaTools.loadAudioClip("bounce.au");
			bounceClip.play();
		}
