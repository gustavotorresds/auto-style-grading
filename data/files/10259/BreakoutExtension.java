...
public class BreakoutExtension extends GraphicsProgram {
	...
	*/
	...
	// Alerts the user when they lose a life, updates the number of lives left and pauses for 2.5
	// seconds before the next round starts.
		if(!gameWon() && nTurnsLeft >0) {
			nTurnsLeft--;
			remove(ball);
			GLabel lifeLost = new GLabel("You lost a life. Lives remaining: " + nTurnsLeft);
			add(lifeLost, (getWidth()- lifeLost.getWidth())/2, (getHeight()-lifeLost.getHeight())/2);
			pause(2500);
			remove(lifeLost);
		
		// Congratulates winner by rewarding them with a picture of my RA.
		} else if(gameWon()) {
			removeAll();
			congratsFromIan();
		}
	}
	private void congratsFromIan() {
		GImage ian = new GImage("ian.jpg");
		double ianWidth = ian.getWidth();
		double ianHeight = ian.getHeight();
		double ianRatio = ianWidth/ianHeight;
		double scaledWidth = CANVAS_WIDTH;
		double scaledHeight = scaledWidth/ianRatio;
		ian.setSize(scaledWidth, scaledHeight);
		add(ian);
		GLabel winner = new GLabel("You win! Ian congratulates you");
		winner.setFont((new Font("Serif", Font.BOLD, 18)));
		winner.setColor(Color.GREEN);
		add(winner, (getWidth()- winner.getWidth())/2, ((getHeight()-winner.getHeight())/4));
		pause(3*DELAY);
	}
