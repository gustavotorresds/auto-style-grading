 ...
 * 
 * This file implements the game of Breakout. It sets the game up and 
 * the user can click the screen to start playing. 
 */
...
public class Breakout extends GraphicsProgram {
	...
	/*
	 * Sets the game up by adding the bricks and adding the paddle to the screen.
	 */
	...
	/*
	 * Creates the ball, sets it into motion, makes it bounce, checks for collision,
	 * and reset the velocity of the ball accordingly.
	 */
	private void playGame() {
		GOval ball = drawBall();
		vx = rgen.nextDouble(VELOCITY_X_MIN, VELOCITY_X_MAX); // The velocity of the ball
		...
		while (true) {
			...
			ball.move(vx, vy); // velocity and bouncing motion
			if (hitTopWall(ball)) {
				vy = -vy;
			}
	...
	/*
	 * Checks if the colliding object is the paddle or the bottom wall. If the
	 * paddle the game continues and if not, they lose a turn out of the initial
	 * three.
	 */
	...
	private GObject getCollidingObject() {
		GObject possibleCollider1 = getElementAt(ball.getX(), ball.getY()); // top left
		GObject possibleCollider2 = getElementAt(ball.getRightX(), ball.getY()); // top right
		GObject possibleCollider3 = getElementAt(ball.getX(), ball.getBottomY()); // bottom left
		GObject possibleCollider4 = getElementAt(ball.getRightX(), ball.getBottomY()); // bottom right
		if (possibleCollider1 != null) {
			return possibleCollider1;
		} else if (possibleCollider2 != null) {
			return possibleCollider2;
		} else if (possibleCollider3 != null) {
			return possibleCollider3;
		} else if (possibleCollider4 != null) {
