 ...
 * 
 * EXTENSIONS: 
 * 1) start/game over messages, with a click to play again.
 * 2) adds the bounce sound (when bouncing on the paddle or hitting the bricks)!
 * 3) bounce works if it hits the left/right sides of the paddle.
 * 4) (relatively mild) score input, of 100 * each brick in the corner!
 * 
 * Note: strangely, it was extension #4 that cost the most hours (score and label required much finessing).
 */
...
public class BreakoutExtended extends GraphicsProgram {
	...
	public static final double VELOCITY_Y = 5.0; //Originally 3.0. Can be changed for fun.
	...
	//I created this for score font.
	public static final String SCORE_FONT = "SansSerif-BOLD-13";
	
	//Instance variables
	private RandomGenerator rgen = RandomGenerator.getInstance();
	private double vx;
	private double vy;
	private int brickCount; //Initializes # of bricks in beginning (universal).
	...
	//Note: this was declared only so I could set the ball to ignore the GLabel.
	//If not, then if the ball collided with it, it would inflate the score.
	private GLabel scoreLabel = new GLabel("");

	//This was added just so I could print it in the end.
	private int score; 
	...
	private void collideWithSomething() {
		...
		if(collObj == paddle) { //As in, finding the reference!
			...
			//Extra feature: ball bounces on the LEFT side of the paddle. Sticky paddle checked (see above).
			//It's harder to hit the side, so I increased the range in which the ball bouncing still worked.
			if (ball.getX() >= paddle.getX() -2*BALL_RADIUS && ball.getX() - VELOCITY_X_MAX < paddle.getX() - 2*BALL_RADIUS + VELOCITY_X_MAX) {
				vy = -vy;
				vx = -vx;
				bounceClip.play();
			}
			
			//Extra feature: ball bounces on the RIGHT side of the paddle. Sticky paddle checked (see above).
			//It's harder to hit the side, so I increased the range in which the ball bouncing still worked.
			//Don't need to account for ball radius (checks left upper corner)
			if (ball.getX() <= paddle.getX() + PADDLE_WIDTH + VELOCITY_X_MAX && ball.getX() > paddle.getX() + PADDLE_WIDTH - VELOCITY_X_MAX) {
				vy = -vy;
				vx = -vx;
				bounceClip.play();
			}
		...
		//Using == "brick" would just work for the last brick made in the loop.
			...
			if(collObj != scoreLabel) { //This checks so the ball doesn't bounce off the score label!
				remove(collObj);
	...
	private void setUp() {
		...
		setBall();
		addMouseListeners();
		brickCount = NBRICK_ROWS * NBRICK_COLUMNS; //Sets the bricks here so score can refresh later. 
	}
...
//These four methods are part of extensions (and thus not ordered as above).
//The next three messages are "technically" extensions, but I included them in base game.
	...
	//This is used to update the score. The numbers are odd as I added it in the corner as best I could.
	private void updateScore() {	
		score = (NBRICK_COLUMNS*NBRICK_ROWS - brickCount) * 100;
		scoreLabel.setFont(SCORE_FONT);
		add(scoreLabel, getWidth() - PADDLE_WIDTH*1.3, getHeight() - PADDLE_Y_OFFSET/3);
