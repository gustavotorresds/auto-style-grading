 ...
 * File purpose: Implements the game of Breakout.
...
public class Breakout extends GraphicsProgram {
	 ...
	 */
	public void run() {
		// sets up breakout world
		setUp();
		// brick counter
		int counter = 100;
		// random generator for vx ball direction
		if (rgen.nextBoolean(0.5)) vx = -vx;
		//ball starts moving when mouse clicked
		waitForClick();
		// while loop to run ball movement and collision checker which win and 
		//game over conditions are not satisfied
		while (!gameOver() && !win(counter)) {
			// allows the ball to bounce around the canvas
			moveBall();
			// checks objects the ball collides with
			counter = checkForBallCollisions(counter);
		}
		// indicates if user won or lost
		wonLostCondition();
	}

	/*
	 * Sets up world of breakout
	 * 
	 * Pre: Empty world
	 * Post: Set window size with bricks, paddle, and ball
	 */
	...
	/*
	 * Makes ball bounce off top wall, right wall, left wall, and paddle. 
	 * Eliminates brick if it bounces off brick. Ball continues to move
	 * until last brick is eliminated.
	 * 
	 * Pre: Stationary ball
	 * Post: Ball that bounces off top wall, right wall, left wall
	 */
	...
	private int checkForBallCollisions(int counter) {
		// retrieves object ball collides with
		GObject collider = getCollidingObject();
		if (collider != null) {
			// ball bounces off paddle
			if (collider == paddle) {
				vy = -Math.abs(vy);
				// ball bounces off brick, brick disappears, counter records one less 
				// brick
			} else {
				remove(collider);
				vy = -vy;
	...
	/*
	 * Returns whether or not ball should ounce off the top wall of the window 
