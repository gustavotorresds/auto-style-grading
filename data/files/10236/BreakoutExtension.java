...
public class BreakoutExtension extends GraphicsProgram {
	...
	public void run() {
		...
		if(playerWins(numberOfBricksRemaining)) {
	...
	private void setup(int livesRemaining) {
		...
		makePaddle();
		makeBall();						
		makeLifeCounter(livesRemaining);
		waitForClick();
	}
	...
	/*
	 * Makes a label that displays "Lives: " and the number of lives remaining
	 */
	private void makeLifeCounter(int livesRemaining) {
		double lifeCounterX = getWidth() - 20;
		double lifeCounterY = getHeight() - 10;
		lifeCounter = new GLabel(Integer.toString(livesRemaining), lifeCounterX, lifeCounterY);
		add(lifeCounter);
	}
	
	/*
	 * Displays a congratulatory message. For winners' eyes only
	 */
	private void winnerWinnerChickenDinner() {
		...
		rect.setFilled(true);
		add(rect);
		GLabel winMessage = new GLabel(CONGRATS);
		double winMessageX = getWidth()/2 - winMessage.getWidth()/2;
		double winMessageY = getHeight()/2 + winMessage.getHeight()/2;
		winMessage.setColor(Color.YELLOW);
		add(winMessage, winMessageX, winMessageY);
	}
	
	/*
	 * Displays a loss message when player loses
	 */
	private void completeDisgrace() {
		removeAll();
		GRect rect = new GRect(getWidth(), getHeight());
		rect.setFilled(true);
		add(rect);
		GLabel lossMessage1 = new GLabel(SUCKS_TO_SUCK_1);
		double lossMessage1X = getWidth()/2 - lossMessage1.getWidth()/2;
		double lossMessage1Y = getHeight()/2;
		GLabel lossMessage2 = new GLabel(SUCKS_TO_SUCK_2);
		double lossMessage2X = getWidth()/2 - lossMessage2.getWidth()/2;
		double lossMessage2Y = getHeight()/2 + lossMessage2.getHeight();
		lossMessage1.setColor(Color.YELLOW);
		lossMessage2.setColor(Color.YELLOW);
		add(lossMessage1, lossMessage1X, lossMessage1Y);
		add(lossMessage2, lossMessage2X, lossMessage2Y);
	}
		...
		while(livesRemaining > 0 && !playerWins(numberOfBricksRemaining)) {
			...
			if(getCollidingObject() != null) {
				...
				if(collidingObject == paddle) {
					...
					if(numberOfPaddleBounces == 7) {
						vx *= 2;						//The kicker. After the ball has bounced off the paddle 7 times, the horizontal velocity doubles
					}
				} else if(collidingObject != lifeCounter){
					verticalBounce();
			...
			if(ballHitBottom()) {					//Controls what happens when the player loses a life
				...
				remove(lifeCounter);					//Life counter disappears
				makeBall();							//New ball is added in the center
				makeLifeCounter(livesRemaining);		//The life counter is remade with a new value to reflect the correct life number
				numberOfPaddleBounces = 0;
				vx /=2;
				waitForClick();
			}
		}
