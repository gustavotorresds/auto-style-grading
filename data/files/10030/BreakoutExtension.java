...
public class BreakoutExtension extends GraphicsProgram {
	...
	//The following instance variables are fairly self-explanatory.
	...
	private double vx = rgen.nextDouble(1.0, 3.0); { 
		if (rgen.nextBoolean(.05)) vx = -vx; // This calls the random generator to make the x value negative half the time.
	}
	...
	public void mouseMoved(MouseEvent e) {// This allows the entire program to track where the mouse is.
		...
		if ((mouseX + PADDLE_WIDTH)< CANVAS_WIDTH) { // This if statement ensures that the paddle's location is only set for bounds within the screen.
			paddle.setLocation(mouseX, getHeight() -PADDLE_Y_OFFSET);
			// Because the "run" method stops running temporarily and gives its functionality to the mouseMoved method, the paddle must be created inside the mouse movement method in order for it to track the mouse's movement.
	...
	private void createBricksAtTop() {
		for (int a=0; a<NBRICK_COLUMNS; a++) {
			double initialX = ((CANVAS_WIDTH/2)/ -(BRICK_WIDTH *NBRICK_ROWS) - (BRICK_SEP-1 * NBRICK_ROWS) -BRICK_SEP/2); // The initialx variable allows the program to find the first value of x that will cause the brick block to be centered.
			for (int i= 0; i<NBRICK_ROWS; i++) {
				...
				add(brick);
				if (a<2){ // the if statements allow the rows to take on different colors without having to rewrite a for loop for every separate color. 
	...
	private void createIntroLabel() {
		...
		add(label);
		GLabel label2 = new GLabel ("For an extra challenge, double-click to make the ball go faster. Good luck!", BRICK_WIDTH/2, getHeight()-40);
		add(label2);
		waitForClick();
		remove(label);
		remove(label2);
	...
	/* This method is responsible for bouncing the ball. It creates an animation loop, and checks to see whether the 
	 * ball has collided with anything. It also checks to see whether the ball has hit one of the side walls. 
	 * If so, it reverses the x direction. If the ball has hit a top or bottom wall, the ball reverses y direction.*/
		...
		while (!hitBottomWall()) { 
			...
			if (collided != null) { // checks that a collision has occurred.
				if (collided == paddle) { // if the ball collides with the paddle, reverse y direction (travel up).
					bounceClip.play();
					vy = - Math.abs(vy);
				}else{ // if the ball collides with something (but doesn't collide with the paddle) remove the object.
					remove(collided);
					vy = -vy;
	...
	/* This method tests whether the ends of the ball's x axis have bumped against the screen.  */
	...
	// This method creates a boolean. If the ball touches the bottom wall, this will return true.
	...
	/* If this method doesn't return null 
	 * at one of the parameters, it will return the value of "collided", telling the bounceBall()
	 * method which corner of the ball collided with an object. */
