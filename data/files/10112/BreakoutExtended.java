/*
 * File: Breakout.java Extended
 * -------------------
 * Name: Amelia O'Donohue
 * Section Leader: Marilyn Zhang
 * 
 * This file runs the extension of the game Breakout. In this game, the ball moves around 
 * the world, bouncing off of the walls. When the ball hits a brick, it removes the brick.
 * When it hits the paddle at the bottom of the screen, the ball changes direction. The 
 * turn ends when either all of the bricks have been removed or when the ball hits the 
 * bottom wall. The player has 3 turns.
 * 
 * This extension tells the player their score (1 point per brick) when they lose. It
 * speeds up the ball as the player hits more bricks to make the game more difficult.
 * Every time the ball hits a paddle or a brick, the program plays a short bounce sound.
 * 
 * 
...
public class BreakoutExtended extends GraphicsProgram {
	...
	/*
	 * This method sets a label that shows up after the player's three turns. It reads "Game Over."
	 */
	private void gameOver() {
		GLabel gameOver = new GLabel ("Game Over.", 5, getHeight()/2 + 60);
	...
	/*
	 * This method displays the player's score after the game ends. If the player has hit less than or
	 * equal to 70 bricks, a label appears on the screen that tells them they lost and says their score.
	 * If they hit more than 70 bricks, a label says that the player has won and says the player's score.
	 */
	private void displayScore(int numBricks) {
		int numBricksHit = (NBRICK_ROWS * NBRICK_COLUMNS) - numBricks;
		if (numBricksHit <= 70) {
			GLabel label = new GLabel ("You lost! Your score was " + numBricksHit, 5, getHeight()/2);
			label.setFont("Courier-24");
			add(label);
		} else {
			GLabel label = new GLabel ("Congratulations! You won.", 5, getHeight()/2);
			label.setFont("Courier-24");
			label.setColor(Color.GREEN);
			add(label);	
			GLabel labelScore = new GLabel ("Your score was: " + numBricksHit, 5, getHeight()/2 + 25 );
			labelScore.setFont("Courier-24");
			labelScore.setColor(Color.GREEN);
			add(labelScore);
		}
	}
	...
	private void moveBall() {
		...
		while(hitBottomWall() == false && numBricks != 0) {
			...
			vy = speedUp(numBricks);
			pause (DELAY);
		}
		displayScore(numBricks);
	}
	
	/*
	 * This method speeds up the ball by changing the y direction velocity as the player
	 * hits more bricks.
	 */
	private double speedUp(int numBricks) {
		if (numBricks == 90) {
			vy = 1.001*vy;
			vx = 1.001*vx;
		}
	...
	/* This method makes a new paddle every time the mouse moves. */
