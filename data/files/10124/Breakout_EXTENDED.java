 ...
 * 1. Bounce audio plays when ball hits brick or paddle.
 * 2. Before each turn, waits for user to click and displays a message.
 * 3. Increases vertical velocity by 0.5 for every ten bricks cleared.
 * 	  (resets each turn.)
 */
...
public class Breakout_EXTENDED extends GraphicsProgram {
	...
	private void playGame() {
		...
		while(ballsRemaining > 0) { //Plays a new turn while there are balls remaining.
			GOval ball = makeBall();
			GLabel startPrompt = new GLabel("Click to begin");
			double promptY = (BRICK_Y_OFFSET + NBRICK_ROWS * (BRICK_HEIGHT + BRICK_SEP) - BRICK_SEP + ball.getY() + startPrompt.getAscent())/2; // Centers label in y direction b/w top of ball and bottom of brick column.
			
			add(ball);
			add(startPrompt, (getWidth() - startPrompt.getWidth())/2, promptY);
			...
			while(true) { // Animation loop for ball.
				...
				vChanged = setVY(bricksRemaining, vChanged); // Increases velocity after every 10th brick is eliminated.
	...
	/**
	 * Checks whether a multiple of ten bricks are remaining, and if they are,
	 * increases vertical velocity by one. vChanged boolean ensures this only
	 * happens once every ten bricks.
	 * @param bricksRemaining
	 * @param vChanged
	 * @return vChanged
	 */
	private boolean setVY(int bricksRemaining, boolean vChanged) {
		if(bricksRemaining % 10 == 0 && !vChanged) {
			vy += 0.5;
			vChanged = true;
		} else if(bricksRemaining % 10 != 0) {
			vChanged = false;
		}
		return vChanged;
	}
