 ...
 * 
 * This file creates an extension to the game "Breakout," where the user will use a paddle (tracked by the mouse) to rebound a ball,
 * to make the ball hit a series of blocks above. Each time the ball hits a block, that block is removed. The user has a set number of 
 * lives. A life is lost when the ball drops below the paddle and falls below the bottom boundary of the window. The user loses the 
 * game when he/she runs out of lives. The user wins the game if he/she clears all of the bricks in the window before losing all of 
 * his/her lives. This extended version of "Breakout" includes a welcome screen with click-to-start options, a lives counter at the bottom
 * of the window, ball bouncing audio when the ball collides with something on the screen, and a return to welcome screen option at the end
 * so that the user may play again.
 */
...
public class ExtensionBreakoutAssignment3 extends GraphicsProgram {
	...
	private void setUpGame() {
		createBricks();
		createPaddle();
		createStartingLivesCounter(NTURNS);
	}
	...
	// Sets the text "Welcome to Breakout"
	private void welcomeToBreakout() {
		setTitle("CS 106A Breakout");
		setCanvasSize(CANVAS_WIDTH, CANVAS_HEIGHT);
		
		GLabel breakout = new GLabel ("WELCOME TO BREAKOUT");
		breakout.setFont("Courier-24");
		breakout.setColor(Color.RED);
		add(breakout, ( getWidth() - breakout.getWidth() ) / 2, getHeight() / 3 - breakout.getAscent() / 2) ;
	}
	
	// Sets the instructions of the game
	private void printInstructions() {
		GLabel instructions = new GLabel ("Move the paddle to hit the ball and clear the bricks!");
		instructions.setColor(Color.BLACK);
		add(instructions, ( getWidth() - instructions.getWidth() ) / 2, getHeight() / 2 - instructions.getAscent() / 2 );
	}
	
	// Sets the text "Click Anywhere To Play"
	private void clickToPlay() {
		GLabel clickToPlay = new GLabel ("Click Anywhere To Play");
		clickToPlay.setFont("Courier-24");
		clickToPlay.setColor(Color.GREEN);
		add(clickToPlay, ( getWidth() - clickToPlay.getWidth() ) / 2, 2 * getHeight() / 3 - clickToPlay.getAscent() / 2 );
	}
	...
	// Creates the counter for how many lives the user starts with
	private void createStartingLivesCounter(int numberOfLives) {
		livesCounter = new GLabel ("Lives: " + numberOfLives);
		double x = ( getWidth() - livesCounter.getWidth() ) / 2;
		double y = getHeight() - livesCounter.getAscent();

		livesCounter.setLocation(x, y);
		add(livesCounter);
	}
	...
	// Updates the lives counter if ballNotTouchingBottom is false
	private void updateLivesCounter() {
		lives--;
		livesCounter.setLabel("Lives: " + lives);
	}
	...
	private void brickCollision(GObject collider) {
		if (collider != null && collider != paddle && collider != livesCounter) {
			bounceClip.play();
			bricksRemaining--;
			vy = -vy;
			remove(collider);
	...
	private void displayWinMessage() {
		...
		add(winMessage, ( getWidth() - winMessage.getWidth() ) / 2, getHeight() / 3 - winMessage.getAscent() / 2);
	}
	...
	private void displayLostMessage() {
		...
		add(gameOver, ( getWidth() - gameOver.getWidth() ) / 2, getHeight() / 3 - gameOver.getAscent() / 2);
	}

	// Allows the user to return to the welcome screen by clicking the mouse
	private void addPlayAgainOption() {
		GLabel playAgain = new GLabel ("Click anywhere to return to the welcome screen.");
		playAgain.setFont("Courier-12");
		playAgain.setColor(Color.BLACK);
		add(playAgain, ( getWidth() - playAgain.getWidth() ) / 2, 2 * getHeight() / 3 - playAgain.getAscent() / 2);
	}
}