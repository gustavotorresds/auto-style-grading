 ...
 * Extensions: 
 * 	1. I added a start label that tells the player to click to start the game. The 
 * 	label disappears when the player starts the game. 
 * 	2. A label appears that tells the player how many turns they have remaining. The
 * 	label also signifies how to start playing the next turn. 
 * 	3. I added a confetti explosion for when the player wins the game. Some confetti 
 *  is animated and floats off the screen, and more confetti remains in the background. 
 * 
...
public class ExtensionBreakout extends GraphicsProgram {
	...
	// Instance variable for the start label.
	GLabel start = null; 
	...
	// Instance variable for number of bricks. 
	...
	private void playGame() {
		...
		for(int i = 0; i < NTURNS; i++) {
			...
			resetBall(); 
			turns--; 
			alertRemainingTurns(turns); //Extension tells how many turns remain. 
		}
		remove(ball); 
		printResult(gameWon); 
		// Adds celebratory confetti when the player wins. 
		for(int c = 0; c < 50; c++) {
			confetti = makeCelebratoryConfetti();
		}
		// Animates some specific confetti to make confetti explode onto screen. 
		GRect C1 = makeCelebratoryConfetti(); 
		GRect C2 = makeCelebratoryConfetti(); 
		GRect C3 = makeCelebratoryConfetti(); 
		GRect C4 = makeCelebratoryConfetti(); 
		GRect C5 = makeCelebratoryConfetti(); 
		GRect C6 = makeCelebratoryConfetti(); 
		GRect C7 = makeCelebratoryConfetti(); 
		GRect C8 = makeCelebratoryConfetti(); 
		GRect C9 = makeCelebratoryConfetti(); 
		GRect C10 = makeCelebratoryConfetti(); 
		GRect C11 = makeCelebratoryConfetti(); 
		GRect C12 = makeCelebratoryConfetti(); 
		GRect C13 = makeCelebratoryConfetti(); 
		GRect C14 = makeCelebratoryConfetti(); 
		GRect C15 = makeCelebratoryConfetti(); 
		GRect C16 = makeCelebratoryConfetti(); 
		GRect C17 = makeCelebratoryConfetti(); 
		GRect C18 = makeCelebratoryConfetti(); 
		GRect C19 = makeCelebratoryConfetti(); 
		GRect C20 = makeCelebratoryConfetti();
		animateConfetti(C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13, C14, C15, C16, C17, C18, C19, C20); 
	}

	/* This method tells the user to click on the screen in order to start the game.
	 * 
	 * Precondition: There is nothing on the screen.
	 * Postcondition: There is a label that disappears when the game starts.
	 */
	private void startLabel() {
		start = new GLabel (" Click to Start "); 
		add(start, getWidth()/2 - start.getWidth()/2, BRICK_Y_OFFSET - 10); 
	}
	...
	/* This method flashes the remaining turns before starting the game over.
	 * 
	 * Precondition: The ball resets but does not notify the player how many turn remain.
	 * Postcondition: A label appears to signify the turns that remain.
	 */
	private void alertRemainingTurns(int turns) {
		if(turns != 0) {
			GLabel remainingTurns = new GLabel(turns + " turn(s) remaining. Click twice to start next turn."); 
			add(remainingTurns, getWidth()/2 - remainingTurns.getWidth()/2, BRICK_Y_OFFSET - BRICK_Y_OFFSET/2); 
			waitForClick(); 
			remove(remainingTurns); 
		}
	}
	...
	/* This method creates confetti that fills the screen when the player wins.
	 * 
	 * Precondition: The player wins and a label appears.
	 * Postcondition: The player wins and a label with confetti appears. 
	 */
	public GRect makeCelebratoryConfetti() {
		int width = 20;
		int height = 5; 
		double xcoord = rgen.nextDouble(0, getWidth()-width); 
		double ycoord = rgen.nextDouble(0, getHeight()-height); 
		GRect confetti = new GRect(xcoord, ycoord, 20,5); 
		confetti.setFilled(true);
		confetti.setColor(rgen.nextColor());
		add(confetti);
		return(confetti); 
	}
	/* This method animates the confetti to fall and through the screen.
	 * 
	 * Precondition: The confetti appears but does not explode through the screen.
	 * Postcondition: Some confetti moves about the screen to have a celebratory effect. 
	 */ 
	private void animateConfetti(GRect C1, GRect C2, GRect C3, GRect C4, GRect C5, GRect C6, GRect C7, GRect C8, GRect C9, GRect C10, GRect C11, GRect C12, GRect C13, GRect C14, GRect C15, GRect C16, GRect C17, GRect C18, GRect C19, GRect C20) {
		while(true) {
			C1.move(vx,vy);
			C2.move(vx,vy);
			C3.move(vx,vy);
