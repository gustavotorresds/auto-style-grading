...
public class ExtensionBreakout extends GraphicsProgram {
	 ...
	 * Each brick is added in the innermost loop. They are coloured separately.
	 */
	 ...
	 * This will:
	 * 1. Print a label requesting that the ball be clicked to begin
	 * 2. Wait for the ball to be clicked before setting it off and removing the label
	 * 3. Initialise the values of vx and vy appropriately
	 * 4. Update the position of the ball after a delay
	 * And continuously:
	 * 5. Make sure the ball bounces off the walls correctly
	 ...
	 * 6. Check if the ball has collided with an object and deal with the object appropriately
	 * 7. Once the game has ended, print whether the game has been lost or won on the canvas
	 */
	 ...
	 * This is based on the code for the bouncing ball from lecture 9.
	 */
	...
	// Private method #2b-v: Wait for ball to be clicked.
	private void waitForBallClick() {
		GLabel clickBallToBegin = new GLabel("Click ball to begin");
		clickBallToBegin.setFont(SCREEN_FONT);
		add(clickBallToBegin, getCenterX()-0.5*(clickBallToBegin.getWidth()), 0.5*BRICK_Y_OFFSET);
		waitForClick();
		remove(clickBallToBegin);
	}

	/*
	 * Private method #2b-vi: Print you win or you lose, depending on whether:
	 * 1. The player is out of turns (loss)
	 * 2. The player is out of bricks (win)
	 */
	private void youWinYouLose () {
		if (bricksRemaining == 0) {
			GLabel youWin = new GLabel("You win :)");
			youWin.setFont(SCREEN_FONT);
			add(youWin, getCenterX() - (youWin.getWidth()/2), getCenterY() - (youWin.getAscent()/2));
		} else {
			GLabel youLose = new GLabel("You lose :(");
			youLose.setFont(SCREEN_FONT);
			add(youLose, getCenterX() - (youLose.getWidth()/2), getCenterY() - (youLose.getAscent()/2));
		}
	}
