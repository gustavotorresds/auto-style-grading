 ...
 * 
 * Valentines day is coming up so I figured I would make you my 
 * break out homescreen (no homo)
 * 
 * The ball is an homage to the oreos you brought on the first day 
 */
...
private void breakOut() {
	...
	for (int i = 0; i < NTURNS; i++) {
		waitForClick();
		if (ball == null) {
			ball = new GImage ("Android_Oreo_Icon.png", getWidth()/2 + BALL_RADIUS, getHeight()/2 - BALL_RADIUS);
			ball.setSize(3*BALL_RADIUS,3*BALL_RADIUS);
			add(ball);
	...
	if (brickAmount != 0) {
		...
		gameOver.setLocation(getWidth()/2 - gameOver.getX(),getHeight()/2+ gameOver.getX());
		add(gameOver);
	}
		else {
	 //win message
		GLabel dubCity = new GLabel ("CONGRADULATIONS YOU WON");
			dubCity.setLocation(getWidth()/2 - 2* dubCity.getWidth(),getHeight()/2+ 2* dubCity.getHeight());
			add(dubCity);
	}
}
 ...
 * @return
 */
private boolean hitFloor(GImage ball2) {
	return ball2.getY() > getHeight() - 2 * BALL_RADIUS;
}
private boolean hitCeiling(GImage ball2) {
	return ball2.getY() <= 0 + BALL_RADIUS;
}
private boolean hitRightWall(GImage ball2) {
	return ball2.getX() >= getWidth() - 2 * BALL_RADIUS;
}
private boolean hitLeftWall(GImage ball2) {
...
private void buildGameBoard() {
	...
	GImage datBoi = new GImage ("18422424_1323033574401218_6247776205410016537_o.jpg", 0,0);
	datBoi.setBounds(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
	datBoi.sendToBack();
	datBoi.setVisible(true);
	add(datBoi);
