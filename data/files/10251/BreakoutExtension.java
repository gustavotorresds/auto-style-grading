...
public class BreakoutExtension extends GraphicsProgram {
	...
	public void run() {
		...
		for(int turn=0;turn<NTURNS;turn++) {
			...
			waitForClick();
			playBall();
			//ends the game when tiles run out 
			if(tilesLeft==0) {
				turn=NTURNS;
				declareWinner();	
			}
	...
	private void declareWinner() {
		...
		win.setFont(new Font("Handwriting", Font.BOLD, 30));
		double y = getHeight()/2.0 - win.getAscent()/2;
		double x = getWidth()/2.0 - win.getWidth()/2;
		win.setLocation(x,y);
		add(win);
	
		//flashes win label on screen 
			for(int i=0;i<6;i++) {
	...
	private void playAgain() {
		play = new GLabel("Click to Play Again");
		play.setFont(new Font("Handwriting", Font.BOLD, 20));
		double y = (getHeight()/2.0 - play.getAscent()/2)+90;
		double x = getWidth()/2.0 - play.getWidth()/2;
	...
	private void playBall() {
		...
		while(ball!=null) {
			ball.move(vx,vy);
			if(paddleBounce<=6) pause(DELAY);
			if(paddleBounce>6 && paddleBounce<=15) pause(DELAY/1.3);
			if(paddleBounce>15&& paddleBounce<=25) pause(DELAY/1.6);
			if(paddleBounce>25) pause(DELAY/1.9);
	...
	public void mouseMoved(MouseEvent e) {
		...
		//make "play again?" label red when scrolled over 
		play.setColor(Color.BLACK);
		GLabel move= getElementAt(x,y);
		if(move==play) {
	...
	private void resetGame() {
		removeAll();
		tilesLeft = NBRICK_COLUMNS*NBRICK_ROWS;
		paddleBounce = 0;
		run();
