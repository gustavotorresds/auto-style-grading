 ...
 * 
 * This file implements the game of Breakout with extensions. The objective of the game is to hit all bricks. 
 ...
 * Extensions: 
 * - Add a bounce noise every time the ball bounces off the walls, a brick or the paddle.
 * - The paddle has been modified so that it is a triangle, with one vertex pointing towards the bottom of the canvas.
 */
...
public class BreakoutExtension extends GraphicsProgram {
	...
	// Dimensions of the paddle, which is now a triangle
	public static final double TRIANGLE_HEIGHT = 20;
	public static final double TRIANGLE_WIDTH = 60;
	...
	// Instance variable allowing for the program to emit a bouncing noise every time the ball 
	// hits the paddle, a brick or a wall.
	...
	/*
	 * Method: Create the paddle. In the extension file, the paddle is a triangle.
	...
	private void createPaddle() {
		paddle = new GPolygon();
		paddle.addVertex(0, TRIANGLE_HEIGHT / 2);
		paddle.addVertex(0 - TRIANGLE_WIDTH / 2, - TRIANGLE_HEIGHT / 2);
		paddle.addVertex(TRIANGLE_WIDTH / 2, - TRIANGLE_HEIGHT / 2);
	...
	public void mouseMoved(MouseEvent e) {
		double x_paddle = 0;
		if (e.getX() <= (getWidth() - TRIANGLE_WIDTH)) {
			x_paddle = e.getX() + TRIANGLE_WIDTH / 2;
		} else {
			x_paddle = getWidth() - TRIANGLE_WIDTH / 2;
		}
	 ...
	 * This method also includes an extension. Every time the number of bricks remaining is divisible by five, 
	 * both the magnitude of vx and of vy increase by five percent.
	...
	private void checkCollisions() {
		...
		if (collider != null) {
			bounceClip.play();
			if (collider == paddle) {
				if (ball.getY() <= (getHeight() - TRIANGLE_HEIGHT - PADDLE_Y_OFFSET) && vy >= 0) {
					vy = -Math.abs(vy);
				} 
			} else {
				vy = -vy;
