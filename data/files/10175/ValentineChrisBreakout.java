...
public class ValentineChrisBreakout extends GraphicsProgram {

	private static final long serialVersionUID = -1528899030543208978L;
	...
	//final new colors
	public final static Color LIGHTEST_PINK = new Color(255,228,225);
	public final static Color SEASHELL_PINK = new Color(255,192,203);
	public final static Color HOT_PINK = new Color(255,20,147);
	public final static Color PURPLE_PINK = new Color(199,21,133);
	public final static Color MULBERRY_PINK = new Color(220,20,60);

	//add bounce file
	AudioClip bounceClip = MediaTools.loadAudioClip("bounce.au");

	//sets up the brick tracker on the screen
	private GLabel pointsLeft = new GLabel("");

	public void run() {
		addMouseListeners();
		// Set the window's title bar text
		setTitle("Chris Peich Valentines Breakout");
		...
		if (brickCount !=0) {
			add(createLabel("Chris doesn't love you:-(",banner));
		}else {
			addBackground();
			add(createLabel("Chris wants you to be his Valentine",banner));
		}
	}
	...
	//adds the number of bricks needed to get Chris to be player's Valentine
	private void addPointsLeft(int n) {
		pointsLeft.setFont("Courier-24");
		pointsLeft.setColor(SEASHELL_PINK);
		pointsLeft.setLabel(brickCount+"bricks left<3.");
		add(pointsLeft,20,getHeight()-20);
		
	}

	private void addStartSign() {
		GRect sign=new GRect(getHeight()/8,getHeight()/2,100,20);
		sign.setFilled(true);
		sign.setColor(LIGHTEST_PINK);
		add(createLabel("Get rid of blocks to find your Valentine ;-)",sign));
		sign.sendToFront();
		
	}

	//adds valentines day background using an image I created in GIMP
	private void addBackground() {
		GImage background = new GImage("valentinechris.jpg");
	...
	private void moveBall() {
		...
		while (true) {
			...
			if (collider == paddle) {
				...
				bounceClip.play();
				vy=-vy;
				//if a brick is hit and then broken, then the ball speeds up.
				vy=vy+.25;
				brickCount--;
				if (brickCount==0) {
					break;
	...
	//if only one row is available to be created, a  single row is created with the # of bricks in a row
	...
	private Color findColor(int a) {
		if ((a*6)%5==0) {
			 ...
			 * so the last remaining value must be Mulberry Pink. without the else, 
			 * the remainder for this
			 * would be 4.
			 */
			return MULBERRY_PINK;
	}
