 ...
 * tethering itself to the paddle. Fire is contagious, and yes, homing balls can catch "on fire,"
 * but only by hitting a red brick. This program makes heavy use of parameters. Smoothest gameplay
...
public class BreakoutExtensionsTest extends GraphicsProgram {
	...
	public static final double VELOCITY_Y_BACKGROUND = 1.0;
	public static final double VELOCITY_X_BACKGROUND = 1.0;
	...
	private AudioClip heychrisClip = MediaTools.loadAudioClip("heychris.mp3");
	private AudioClip heyaveryClip = MediaTools.loadAudioClip("heyavery.mp3");
	private AudioClip screamClip = MediaTools.loadAudioClip("scream.wav");
	...
	public void run() {
		setUp();
		while (!gameOver()) {
			moveBall(ball1, vx1, vy1);
			moveBall(ball2, vx2, vy2);
			moveBall(ball3, vx3, vy3);
			moveBall(ball4, vx4, vy4);
			moveBall(ball5, vx5, vy5);
			moveBackground(chris, vxc, vyc);
			moveBackground(avery, vxa, vya);
			moveBackground(trex, vxt, vyt);
			...
			checkForCollisionsBackground(chris, vxc, vyc);
			checkForCollisionsBackground(avery, vxa, vya);
			checkForCollisionsBackground(trex, vxt, vyt);
			pause(newDelay);
		}
		endResult();
	}
	...
	private void moveBall(GOval ball, double vx, double vy) {
		if (ball != null) {
	...
	// individually tests the four corners of an object for objects until one is found 
	private GObject getCollidingObject(GObject object) {
		GObject collider = null;
		if (collider == null) {
			collider = getElementAt(object.getRightX(), object.getY());
		}
		if (collider == null) {
			collider = getElementAt(object.getX(), object.getY());
		}
		if (collider == null) {
			collider = getElementAt(object.getX(), object.getBottomY());
		}
		if (collider == null) {
			collider = getElementAt(object.getRightX(), object.getBottomY());
		}
		return(collider);
	}
	...
	// this obeys the laws of incidence and reflection
	private void moveBackground(GImage image, double vx, double vy) {
		if (image != null) {
	...
	private void checkForCollisionsBackground(GImage image, double vx, double vy) {
		if (image != null) {
			GObject collider = getCollidingObject(image);  // contacted object will generically be named "collider"
			if (collider != null) {
				backgroundAudio(image, collider);
				//if (image == chris) { vyc = -vyc; }
				//if (image == avery) { vya = -vya; }
				//if (image == trex) { vyt = -vyt; }
			}
	...
	private void backgroundAudio(GImage image, GObject collider) {
		int x = rgen.nextInt(1, 2);
		if (image == chris) {
			if (collider == avery) {
				if (x % 2 == 0) {
